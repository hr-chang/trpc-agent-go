...
排期原则上从业务方感受出发：优先让已落地的 session 外存能力变得可用、可查、可运维，再扩展到更多复制面；工程上保持基本完整性，避免在读取策略、artifact 生命周期和存量评估还不清晰时过快扩大 artifact/ref 生产面。
...
- `session.Events` 中标准 `ContentParts` inline 多模态内容的 persisted view 治理。
- artifact 引用和元信息契约。
- session replay / hydrate 能力。
- 当前运行时与持久化视图分离。

### P1：下一阶段重点路径
- Tool Result / Execution Output 表示治理：ClaudeCode Read、通用 tool result JSON、Skill / CodeExecutor / workspace / subagent 输出文件。
- Telemetry / Debuglog / ExecutionTrace 默认止血：先避免观测和调试系统复制完整多模态 payload。
- Graph Checkpoint / State / HITL Payload 泄漏守护：checkpoint、interrupt/resume、子图 relay、graph state snapshot。
- Workspace / Sandbox / Skill 文件产物治理：workspace dereference、remote CodeExecutor、per-session workspace、Skill 输出与 artifact/workspace ref。
- AG-UI / Client Replay 多模态治理：AG-UI track、MessagesSnapshot、带存储/回放语义的 client replay 面。
