...
- 需求包拆解：
    - [`req-breakdown.md`](./req-breakdown.md)
    - 将整体规划拆成可发布的需求包，只保留需求层面的目标、范围、边界、依赖和验收口径。
- 需求包技术细节：
    - [`req-package-details/`](./req-package-details/)
...
- 评审需求拆分：
    - 先读 `req-breakdown.md`。
    - 对具体链路或代码事实有疑问时，再读 `req-package-details/` 下对应需求包文档。

## 分层原则
- `overview.md` 负责回答“为什么做、整体边界是什么”。
- `storage-path-inventory.md` 负责回答“有没有盘全、内容从哪里来到哪里去”。
- `req-breakdown.md` 负责回答“拆成哪些可发布需求包”。
- `req-package-details/` 负责回答“这些需求包对应哪些代码事实和实现关注点”。
