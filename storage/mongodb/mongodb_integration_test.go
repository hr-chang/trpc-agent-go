...
//
// Set MONGO_URI to a reachable MongoDB deployment to enable these tests.
// For tests that exercise multi-document transactions, the deployment must
// be a replica set or sharded cluster (transactions are not supported on
// standalone servers).
...
func TestIntegration_CRUD(t *testing.T) {
	...
	// InsertOne / FindOne round-trip.
	_, err := client.InsertOne(ctx, db, coll, bson.M{"_id": "a", "v": 1})
	...
	err = client.FindOne(ctx, db, coll, bson.M{"_id": "a"}).Decode(&doc)
	require.NoError(t, err)
	assert.Equal(t, "a", doc.ID)
	assert.Equal(t, 1, doc.V)

	// InsertMany.
	manyRes, err := client.InsertMany(ctx, db, coll, []any{
		bson.M{"_id": "b", "v": 2},
		bson.M{"_id": "c", "v": 3},
	})
	require.NoError(t, err)
	assert.Len(t, manyRes.InsertedIDs, 2)

	// CountDocuments.
	n, err := client.CountDocuments(ctx, db, coll, bson.M{})
	require.NoError(t, err)
	...
	updMany, err := client.UpdateMany(ctx, db, coll,
		bson.M{"v": bson.M{"$gte": 2}}, bson.M{"$inc": bson.M{"v": 100}})
	require.NoError(t, err)
	assert.Equal(t, int64(2), updMany.ModifiedCount)

	// FindOneAndUpdate returns the updated document when so configured.
	var after struct {
		ID string `bson:"_id"`
		V  int    `bson:"v"`
	}
	err = client.FindOneAndUpdate(ctx, db, coll,
		bson.M{"_id": "a"},
		bson.M{"$set": bson.M{"v": 1000}},
		options.FindOneAndUpdate().SetReturnDocument(options.After),
	).Decode(&after)
	require.NoError(t, err)
	assert.Equal(t, 1000, after.V)
	...
	// Aggregate count.
	pipeline := []bson.M{{"$group": bson.M{"_id": nil, "total": bson.M{"$sum": "$v"}}}}
	aggCursor, err := client.Aggregate(ctx, db, coll, pipeline)
	require.NoError(t, err)
	defer aggCursor.Close(ctx)
	var aggOut []bson.M
	require.NoError(t, aggCursor.All(ctx, &aggOut))
...
// "transactions are not supported by this deployment". MongoDB driver does
// not export a sentinel for this, so we match on the well-known message
// fragment.
